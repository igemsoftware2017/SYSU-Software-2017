...
class SDinDesign {
    ...
    static typeVal(type) {
        return type === 'inhibition' ? -1 : 1;
    }
    static typeXor(type1, type2) {
        return SDinDesign.typeVal(type1) === SDinDesign.typeVal(type2) ? 1 : -1;
    }
    ...
    get matrix() {
        ...
        parts.forEach((p) => {
            ...
            let found = design.lines.reduce((f, l) => f || l.start === p.cid, false);
            console.log(p.cid);
            if (found === true)
                return;
            ...
            for (let s of start)
                for (let e of end) {
                    ...
                    if (materialDic[s.cid] !== undefined && materialDic[e.cid] !== undefined) {
                        res[partDic[s.cid].index][partDic[e.cid].index] = SDinDesign.typeXor(s.type, e.type);
                        console.log(s.cid, e.cid, SDinDesign.typeXor(s.type, e.type));
                    }

                    // promote/inhibit promoter
                    if (partDic[e.cid].type === 'promoter') {
                        this.findGenerate(e.cid).forEach((data) => {
                            let a = partDic[s.cid].index;
                            let b = partDic[data.cid].index;
                            if (b === undefined)
                                return;
                            res[a][b] = SDinDesign.typeXor(SDinDesign.typeXor(s.type, e.type), data.type);
                        });
                    }
                }
        });
